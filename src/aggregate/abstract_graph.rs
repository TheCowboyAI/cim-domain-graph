...
use crate::abstraction::{EdgeData, GraphImplementation, GraphMetadata, GraphType, NodeData};
use crate::commands::{GraphCommand, GraphCommandError};
use crate::events::{EdgeAdded, EdgeRemoved, NodeAdded, NodeRemoved};
use crate::value_objects::Position3D;
use cim_domain::{AggregateRoot, EdgeId, GraphId, NodeId};
...
impl AbstractGraph {
    ...
    pub fn add_edge(
        ...
        self.graph
            .add_edge(edge_id, source, target, data)
            .map_err(|e| GraphCommandError::InvalidCommand(e.to_string()))
    }
    ...
    ) -> Result<Vec<Box<dyn std::any::Any>>, GraphCommandError> {
        match command {
            GraphCommand::CreateGraph {
                ...
                Err(GraphCommandError::InvalidCommand(
                    "Graph already exists".to_string(),
                ))
            }
            ...
            GraphCommand::RemoveNode { graph_id, node_id } => {
                ...
                let event = NodeRemoved { graph_id, node_id };
            ...
            GraphCommand::AddEdge {
                ...
                self.graph
                    .add_edge(edge_id, source_id, target_id, edge_data)
                    .map_err(|e| GraphCommandError::InvalidCommand(e.to_string()))?;
            ...
            GraphCommand::RemoveEdge { graph_id, edge_id } => {
                ...
                let event = EdgeRemoved { graph_id, edge_id };
    ...
    pub fn get_edge_endpoints(
        &self,
        edge_id: EdgeId,
    ) -> Result<(NodeId, NodeId), GraphCommandError> {
        // Find the edge in the list of edges
        for (eid, _, source, target) in self.graph.list_edges() {
            if eid == edge_id {
                return Ok((source, target));
            }
        }
        Err(GraphCommandError::EdgeNotFound(edge_id))
    }
...
mod tests {
    ...
    fn test_create_context_graph() {
        ...
        let graph = AbstractGraph::new(GraphType::new_context(graph_id, "Test Context Graph"));
    ...
    fn test_add_remove_nodes() {
        let mut graph = AbstractGraph::new(GraphType::new_context(GraphId::new(), "Test Graph"));
