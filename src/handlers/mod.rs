...
pub trait GraphCommandHandler: Send + Sync {
    ...
    async fn handle_edge_command(&self, command: EdgeCommand) -> GraphCommandResult<Vec<GraphDomainEvent>>;
}
...
impl GraphCommandHandler for GraphCommandHandlerImpl {
    async fn handle_graph_command(&self, command: GraphCommand) -> GraphCommandResult<Vec<GraphDomainEvent>> {
        match command {
            ...
            GraphCommand::ChangeNodeMetadata { graph_id, node_id, new_metadata } => {
                ...
                let old_node = graph.nodes().get(&node_id)
            ...
            GraphCommand::AddEdge { graph_id, source_id, target_id, edge_type, metadata } => {
                ...
                // Add edge to graph
                graph.add_edge(edge_id, source_id, target_id, edge_type.clone(), metadata.clone())?;
    ...
    async fn handle_node_command(&self, command: NodeCommand) -> GraphCommandResult<Vec<GraphDomainEvent>> {
        match command {
            NodeCommand::Add { graph_id, node_type, metadata } => {
                let graph_command = GraphCommand::AddNode { graph_id, node_type, metadata };
            ...
            NodeCommand::ChangeMetadata { graph_id, node_id, new_metadata } => {
                let graph_command = GraphCommand::ChangeNodeMetadata { graph_id, node_id, new_metadata };
                self.handle_graph_command(graph_command).await
            }
        }
    }
    ...
    async fn handle_edge_command(&self, command: EdgeCommand) -> GraphCommandResult<Vec<GraphDomainEvent>> {
        match command {
            EdgeCommand::Add { graph_id, source_id, target_id, edge_type, metadata } => {
                let graph_command = GraphCommand::AddEdge { graph_id, source_id, target_id, edge_type, metadata };
                self.handle_graph_command(graph_command).await
            }
...
mod tests {
    ...
    async fn test_add_node_command() {
        ...
        let events = handler.handle_graph_command(add_node_command).await.unwrap();
        assert_eq!(events.len(), 1);
