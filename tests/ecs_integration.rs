...
use cim_domain_graph::{components::*, events::*, systems::*, EdgeId, GraphId, NodeId};
...
fn test_edge_connection_system() {
    ...
    world.resource_mut::<Events<EdgeAdded>>().send(EdgeAdded {
        ...
        relationship: EdgeRelationship::Association {
            association_type: "triggers".to_string(),
        },
        metadata: HashMap::new(),
    });

    // Run the system
    run_system(&mut world, add_edge_system);
...
fn test_node_update_system() {
    ...
    world
        .resource_mut::<Events<NodeUpdated>>()
        .send(NodeUpdated {
            graph_id,
            node_id,
    ...
    assert_eq!(
        metadata.properties.get("updated"),
...
fn test_node_removal_system() {
    ...
    world
        .resource_mut::<Events<NodeRemoved>>()
        .send(NodeRemoved { graph_id, node_id });
...
fn test_workflow_node_type_assignment() {
    ...
    assert_eq!(results.len(), 1);
    let (node_entity, _node_type) = results[0];
    assert_eq!(node_entity.node_id, node_id);
...
fn test_multiple_graphs_isolation() {
    ...
    world
        ...
        .send(GraphCreated {
            ...
            description: String::new(),
            graph_type: Some(GraphType::Knowledge),
            metadata: HashMap::new(),
            created_at: chrono::Utc::now(),
        });
    ...
    let graph1_nodes: Vec<_> = nodes.iter().filter(|n| n.graph_id == graph1_id).collect();
    let graph2_nodes: Vec<_> = nodes.iter().filter(|n| n.graph_id == graph2_id).collect();
